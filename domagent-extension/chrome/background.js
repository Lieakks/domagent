...
importScripts('../../shared/background-lib.js');

const DEFAULT_HOST = DOMAGENT_DEFAULT_HOST;
const DEFAULT_PORT = DOMAGENT_DEFAULT_PORT;
const DEFAULT_PATH = DOMAGENT_DEFAULT_PATH;
 ...
 */
const AUTOMATION_TAB_KEY = DOMAGENT_AUTOMATION_TAB_KEY;

const BADGE = DOMAGENT_BADGE;
...
async function getRelaySettings() {
  const stored = await chrome.storage.local.get(['host', 'port', 'path', 'auth_token'])
...
function isTabEligible(tab) {
  return domAgentIsTabEligible(tab);
}
