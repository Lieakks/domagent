/* DOMAgent — shared content-script functions (browser context) */

const DOMAGENT_OVERLAY_CSS = `
.__da-scan-box {
...
function domAgentEnsureOverlayStyles() {
  if (document.getElementById('__da-style')) return;
  const s = document.createElement('style');
  s.id = '__da-style';
  s.textContent = DOMAGENT_OVERLAY_CSS;
  (document.head || document.documentElement).appendChild(s);
}

function domAgentClearOverlays() {
...
function domAgentCaptureScreenshot() {
  return '__delegate_to_background__';
}

function domAgentClickElement(selector, overlayEnabled = true, opacity = 0.75) {
  domAgentEnsureOverlayStyles();
  domAgentClearOverlays();
...
function domAgentTypeIntoElement(selector, text, overlayEnabled = true, opacity = 0.75) {
  domAgentEnsureOverlayStyles();
  domAgentClearOverlays();
...
function domAgentGetText(selector) {
  const el = document.querySelector(selector);
  return el ? el.innerText : null;
}

function domAgentGetInteractiveElements(cfg = {}) {
  domAgentEnsureOverlayStyles();
  domAgentClearOverlays();
